...
[[nodiscard]] std::thread launch_named_thread(const std::string& name, Callable&& callable, Args&&... args) {
    return std::thread([name, callable, args...]() {
...
std::experimental::optional<typename Map::mapped_type> find(Map& map, const typename Map::key_type& key) {
...
template <typename Map>
std::experimental::optional<typename Map::mapped_type*> find_ptr(Map& map, const typename Map::key_type& key) {
    auto it = map.find(key);
    if (it != map.end()) {
        return &it->second;
    }
...
R clamp(T1 l, R v, T2 u) {
    return std::min(std::max(l, v), u);
}
