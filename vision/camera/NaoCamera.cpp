                           ...
                           const int32_t controlValue)
{
   if (v4lDeviceP)
   {
      ...
      std::cerr << fd << " - Setting id " << controlId << " \033[36m" << CAMERA_CONTROL_VALUE_MAP[controlId] << ": " << controlValue << "\033[0m...";
      int i = 0;
...
NaoCamera::NaoCamera(const char *filename, const IOMethod method, const int format) : filename(filename), io(method), format(format)
{
...
NaoCamera::NaoCamera(Blackboard *blackboard, const char *filename, const IOMethod method,
                     const int format,
                     int dummy, const std::string cameraChoice) : filename(filename), io(method), format(format),
...
{
   readCameraSettings(cameraSettings, cameraChoice);
}

void NaoCamera::readCameraSettings(CameraSettings &settings, std::string cameraName)
{
   settings.hflip = 0;
   settings.vflip = 0;
   settings.brightness = 20;
   settings.contrast = 60;
   settings.saturation = 190;
   settings.hue = 0;
   settings.sharpness = 2;
   settings.backlightCompensation = 0x00;
   settings.exposure = 90;
   settings.gain = 250;
   settings.whiteBalance = 2700;
   settings.exposureAuto = 1;
   settings.autoWhiteBalance = 1;
   settings.autoFocus = 1;
   settings.focusAbsolute = 0;
   settings.exposureAlgorithm = 3;
   settings.aeTargetAvgLuma = 55;
   settings.aeTargetAvgLumaDark = 27;
   settings.aeTargetGain = 128;
   settings.aeMinVirtGain = 32;
   settings.aeMaxVirtGain = 256;
   settings.aeMinVirtAGain = 32;
   settings.aeMaxVirtAGain = 256;
   settings.aeTargetExposure = 10;
   settings.aeUseWeightTable = true;
   settings.aeWeightTableX1 = 0;
   settings.aeWeightTableX2 = 1;
   settings.aeWeightTableY1 = 0;
   settings.aeWeightTableY2 = 1;
}
                                   ...
                                   CameraSettings &settings, std::string cameraName)
{
   ...
   settings.brightness = 20;
   settings.contrast = 60;
   settings.saturation = 180;
   settings.hue = 0;
   settings.sharpness = 2;
   settings.backlightCompensation = 0x00;
   settings.exposure = 13;
   settings.gain = 250;
   settings.whiteBalance = 2700;
   settings.exposureAuto = 13;
   settings.autoWhiteBalance = 1;
   settings.autoFocus = 1;
   settings.focusAbsolute = 0;
   settings.exposureAlgorithm = 3;
   ...
   settings.aeMaxVirtAGain = 256;
   settings.aeTargetExposure = 10;
   settings.aeUseWeightTable = false;
   settings.aeWeightTableX1 = 0;
   settings.aeWeightTableX2 = 1;
   settings.aeWeightTableY1 = 0;
   settings.aeWeightTableY2 = 1;
...
const uint8_t *NaoCamera::read_frame(void)
{
   ...
   case IO_METHOD_USERPTR:
      ...
      for (i = 0; i < n_buffers; ++i)
         if (lastDequeued.m.userptr == (unsigned long)buffers[i].start && lastDequeued.length == buffers[i].length)
            break;
...
void NaoCamera::init_mmap(void)
{
   ...
   for (n_buffers = 0; n_buffers < req.count; ++n_buffers)
   {
      ...
      buffers[n_buffers].start = reinterpret_cast<uint8_t *>(mmap(NULL /* start anywhere */,
                                                                  buf.length,
